import { useContext } from "react";
import { ShoppingCartContext } from "../../Context";
import { CartProductsDataType } from "../../Interfaces/Interfaces";

type Props = {
  product: CartProductsDataType
  index: number
  deleteCartProduct?: (index: number) => void
}

export function CheckoutOrderCard({ product, index, deleteCartProduct }: Props) {
  const { counter, setCounter, cartProducts, setCartProducts } = useContext(ShoppingCartContext)

  const decreaseProductQuantity = () => {
    if (cartProducts[index].quantity === 1) {
      deleteCartProduct?.(index)
    } else {
      cartProducts[index].quantity -= 1
      setCartProducts([...cartProducts])
      setCounter(counter - 1)
    }
  }

  const increaseProductQuantity = () => {
    cartProducts[index].quantity += 1
    setCartProducts([...cartProducts])
    setCounter(counter + 1)
  }

  return (
    <div
      className='h-20 flex gap-4'
    >
      <figure className="w-20 h-20 flex items-center">
        <img
          src={product.images[0]}
          alt={product.title}
        />
      </figure>

      <div className="flex flex-col gap-2 w-full h-full justify-around">
        <div className='flex justify-between items-center font-semibold text-sm'>
          <span className=''>{ product.title }</span>
          <span>${ product.price * product.quantity }</span>
        </div>

        <div className='flex justify-between items-center'>

          <span className="flex items-center">
            { deleteCartProduct && (
              <>
                <button
                  className='h-5 w-5 bg-black text-white font-bold border border-black rounded grid place-content-center'
                  onClick={() => decreaseProductQuantity()}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={4} stroke="#fff" className="w-4 h-4">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M18 12H6" />
                  </svg>
                </button>

                <span className="w-8 text-center">
                  { product.quantity }
                </span>

                <button
                  className='h-5 w-5 bg-black text-white font-bold border border-black rounded grid place-content-center'
                  onClick={() => increaseProductQuantity()}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={4} stroke="#fff" className="w-4 h-4">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m6-6H6" />
                  </svg>
                </button>
              </>
            )}

            {!deleteCartProduct && (
              <span className="text-sm">
                {`${product.quantity} x $${product.price}`}
              </span>
            )}
          </span>

          { deleteCartProduct &&
            (
              <div
                onClick={() => deleteCartProduct?.(index)}
              >
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 cursor-pointer">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                </svg>
              </div>
            )
          }

        </div>
      </div>

    </div>
  )
}